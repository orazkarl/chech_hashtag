...
class IndexView(generic.ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if len(hashtags) == 0:
            ...
            # followers = LockedIterator(followers)
            # i.followers = str(list(followers))
            # print(i.followers)
            # i.save()
            # for profile in followers:
...
class HashtagView(generic.ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        shpion = Shpion.objects.all()[0]
        h = instaloader.Hashtag.from_name(L.context, hashtag)
        if int(h.mediacount) != Hashtag.objects.filter(hashtag=hashtag).count():
        ...
        for post in Post.objects.all():
            ...
            if UserPost.objects.filter(user_id=post.user_id):
                ...
                not_past_posts_bound_shpion = UserPost.objects.filter(user_id=post.user_id)[0].not_past_posts_bound_shpion
                ...
                not_past_posts_bound_shpion_url = UserPost.objects.filter(user_id=post.user_id)[0].not_past_posts_bound_shpion_url
            if UserPost.objects.filter(user_id=post.user_id):
            ...
            userposts = UserPost(user_id=post.user_id, hashtag=hashtag, username=post.username,
                                 ...
                                 not_past_posts_url=not_past_posts_url, not_past_posts_bound_shpion_url=not_past_posts_bound_shpion_url, is_black_list=is_black_list
                                 )
            userposts.save()
        ...
        self.queryset = UserPost.objects.filter(hashtag=hashtag, is_black_list=False, is_bound_shpion=True)
